...
# predict_dict = {'오이':{'취청(50개)':{'중품':False,'상품':False}, '가시계통(1kg)':{'중품':False,'상품':False}, '다다기계통(100개)':{'중품':False,'상품':False}},
#                            '양파':{'햇양파(1kg)':{'중품':False,'상품':False},'양파(1kg)':{'중품':False,'상품':False},'수입(1kg)':{'중품':False,'상품':False}},
#                            '파':{'대파(1kg)':{'중품':False,'상품':False}, '쪽파(1kg)':{'중품':False,'상품':False}},
#                            '호박':{'애호박(20개)':{'중품':False,'상품':False}, '쥬키니(1kg)':{'중품':False,'상품':False}},
#                            '쌀': {'일반계(1kg)':{'중품':False,'상품':False}, '햇일반계(1kg)':{'중품':False,'상품':False}}}
    ...
    for kind in crops_dict[item][0]:
        for rank in crops_dict[item][1]:
            if item=='오이':
                ...
                element_for_predict[item][kind][rank]={'priceIndex':['곡물및식량작물','수도폐기물처리및재활용','음식점및숙박서비스','사업지원서비스','장비용품및지식재산권임대'],'weather':[]}
...
def CreateModel(item, kind, rank):
...
def Predict(item,kind,rank):
    filename = 'regression_model/{0}_{1}_{2}_model'.format(item,kind,rank)
    
    try:
        loaded_model = pickle.load(open(filename, 'rb'))
    except FileNotFoundError:
        CreateModel(item, kind, rank)
        loaded_model = pickle.load(open(filename, 'rb'))

    priceIndex_select_last_str = []
    for i in range(0, len(element_for_predict[item][kind][rank]['priceIndex'])):
        if i==len(element_for_predict[item][kind][rank]['priceIndex'])-1:
            priceIndex_select_last_str.append("LAST(priceIndex.`{}`)".format(element_for_predict[item][kind][rank]['priceIndex'][i]))
        else:
            priceIndex_select_last_str.append("LAST(priceIndex.`{}`),".format(element_for_predict[item][kind][rank]['priceIndex'][i]))
    priceIndex_select_last_str = ''.join(priceIndex_select_last_str)

    weather_select_last_str = []
    for i in range(0, len(element_for_predict[item][kind][rank]['weather'])):
        if i==len(element_for_predict[item][kind][rank]['weather'])-1:
            weather_select_last_str.append("LAST({0}.`{1}`)".format(area_dict[item],element_for_predict[item][kind][rank]['weather'][i]))
        else:
            weather_select_last_str.append("LAST({0}.`{1}`),".format(area_dict[item],element_for_predict[item][kind][rank]['weather'][i]))
    weather_select_last_str = ''.join(weather_select_last_str)

    query = "SELECT {1},{2} FROM priceIndex JOIN {0}"
    current_df = spark.sql(query.format(area_dict[item],priceIndex_select_last_str,weather_select_last_str))

    test_value = []
    for i in current_df.collect()[0]:
        test_value.append(i)

    result = loaded_model.predict([test_value])
    return result ## 예측가격 반환
