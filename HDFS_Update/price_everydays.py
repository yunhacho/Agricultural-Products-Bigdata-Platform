...
###### 앞에 비어있는 애들 한꺼번에 받아와야 하니까 나중에 처리할 때 쓰기
...
def get_dailyPriceByCategoryList(date, category_code, cls_code='02', kg_yn='Y'):
    dateitems=[]
#     for regday in datelist:
    items={}; dics=[]
    for category in category_code:
        response=dailyPriceByCategoryList(category, date, cls_code, kg_yn)
        if isinstance(response['data'], list)==False:
            if response['data']['error_code']=='000':
                for item in response['data']['item']:
                    itemsample={
...
def UpdatePrice():
#     if __name__=="__main__":
    today_ = datetime.datetime.now() - datetime.timedelta(days=1)    # 오늘꺼 안됨 -> 전날 가격
    today = datetime.date(today_.year, today_.month, today_.day)
    datelist=str(today)
#     datelist=dateVal('2000-01-01', '2021-04-12') 나중에 한꺼번에 받아올 때 쓰기
    category_code=['100','200', '300', '400', '600']
